      ...
      <div className="px-4 sm:px-6 md:px-10 pb-6 md:pb-10">
        ...
        <div className="block md:hidden space-y-4">
          <div className="flex items-start gap-3">
            ...
            <h1 className="text-sm font-normal tracking-tight text-gray-900 flex-1">{snax.name}</h1>
          </div>
          <div className="flex items-start gap-3">
            <div className="text-sm font-bold text-black whitespace-nowrap">Price:</div>
            <div className="text-sm text-gray-900">{`$ ${Number(snax.price).toFixed(2)}`}</div>
          </div>
          <div className="w-full max-w-[200px] mx-auto mt-4">
            <img
              src={snax.image_url || "/images/placeholder.jpg"}
              alt={snax.name}
        ...
        <div className="hidden md:flex gap-6 lg:gap-10 items-start">
          <div className="flex-1 flex flex-col gap-6 min-w-0">
            <div className="flex items-start gap-4">
              ...
              <h1 className="text-lg lg:text-xl font-normal tracking-tight text-gray-900">{snax.name}</h1>
            </div>
            <div className="flex items-start gap-4">
              <div className="w-20 lg:w-24 flex-shrink-0 text-lg lg:text-xl font-bold text-black pt-0.5">Price:</div>
              <div className="text-lg lg:text-xl text-gray-900">{`$ ${Number(snax.price).toFixed(2)}`}</div>
            </div>
          </div>
          ...
          <section className="mt-6 md:mt-12 lg:mt-16">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 lg:gap-10 items-start">
              ...
              <div className="md:col-span-9">
                <p className="text-sm md:text-lg lg:text-xl leading-relaxed text-gray-800">{snax.description}</p>
              </div>
            </div>
          </section>
        )}
