...
const SideDishInfo = ({ product, onBack }) => {
  const dish = product;
  const circleColor = dish.color_code || "#FF66FF";
      ...
      <div className="px-4 sm:px-6 md:px-10 pb-6 md:pb-10">
        ...
        <div className="block md:hidden space-y-4">
          <div className="flex items-start gap-3">
            ...
            <h1 className="text-sm font-normal tracking-tight text-gray-900 flex-1">{dish.name}</h1>
          </div>
          <div className="flex items-start gap-3">
          ...
          <div className="w-full max-w-[200px] mx-auto mt-4">
            <img
              src={dish.image_url ? (dish.image_url.startsWith('http') ? dish.image_url : `/images/${dish.image_url}`) : "/images/placeholder.jpg"}
              alt={dish.name}
        ...
        <div className="hidden md:flex gap-6 lg:gap-10 items-start">
          <div className="flex-1 flex flex-col gap-6 min-w-0">
            <div className="flex items-start gap-4">
              ...
              <h1 className="text-lg lg:text-xl font-normal tracking-tight text-gray-900">{dish.name}</h1>
            </div>
            <div className="flex items-start gap-4">
          ...
          <section className="mt-6 md:mt-12 lg:mt-16">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 lg:gap-10 items-start">
              ...
              <div className="md:col-span-9">
                <p className="text-sm md:text-lg lg:text-xl leading-relaxed text-gray-800">{dish.description}</p>
              </div>
            </div>
          </section>
        )}
