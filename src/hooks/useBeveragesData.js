...
export function assignBeverageDisplayIds(items, sortOrder = null) {
  let sorted;
  ...
  return sorted.map((item) => {
    ...
    const display_id = `B${String(counter).padStart(2, '0')}`;
    counter++;
    return { ...item, display_id };
  });
}

export function useBeveragesData() {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!appwriteConfig.projectId || !appwriteConfig.dbId || !appwriteConfig.beveragesCollectionId) {
        setData([]);
        setLoading(false);
        return;
      }
      ...
      try {
        const response = await databases.listDocuments(
          appwriteConfig.dbId,
          appwriteConfig.beveragesCollectionId,
          [Query.limit(100)]
        );
        ...
        if (response.documents.length > 0) {
          ...
          setData(assignBeverageDisplayIds(processedDocs, parsedSortOrder));
          setSource('remote');
        } else {
          setData([]);
          setSource('local');
        ...
        console.error('Failed to fetch Beverages from Appwrite:', err);
        setData([]);
